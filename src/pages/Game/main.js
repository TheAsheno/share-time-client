...
class Content {
    constructor(el) {
        ...
        this.DOM.backCtrl.addEventListener('click', () => gamepage.slideshow.hideContent());
    }
    show() {
        this.DOM.el.classList.add('content__item--current');

        gsap.to([this.DOM.backCtrl, this.DOM.number, this.DOM.title, this.DOM.subtitle, this.DOM.text], {
            duration: 0.8,
            ease: "power3.out",
            delay: 0.4,
            opacity: 1,
    ...
    hide() {
        ...
        gsap.to([this.DOM.backCtrl, this.DOM.number, this.DOM.title, this.DOM.subtitle, this.DOM.text].reverse(), {
            duration: 0.3,
            ease: "power3.in",
            opacity: 0,
            y: 10,
...
class Slideshow {
    constructor(el, images) {
        ...
        this.current = 0;
        this.images = images;
        this.DOM.parentNode = document.getElementById('gamepage');
        this.DOM.parentNode.style.backgroundImage = `url(${BASE_URL}/game/images/${encodeURIComponent(this.images[this.current])})`;
        this.DOM.deco = this.DOM.el.querySelector('.slideshow__deco');
        this.contents = [];
        Array.from(document.querySelectorAll('.game-content > .content__item')).forEach(contentEl => this.contents.push(new Content(contentEl)));
